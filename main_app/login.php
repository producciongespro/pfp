...
// verifica si la peticiòn es de tipo AJAX
  if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])== 'xmlhttprequest'    ) {
    ...
    if ($nueva_consulta = $mysqli->prepare("Select nombre, apellido1, apellido2, correo,  id_instancia, id_tipo, password From usuarios Where correo = ?")) {
        ...
        if ($resultado->num_rows == 1) {
            ...
            if ((password_verify($pas, $encriptado_db) AND (isActivo($usuario))))
            {
              ...
              $_SESSION['usuario'] = $datos;
                echo json_encode(
						array('error'=>false,'nombre'=>$datos['nombre'], 'apellido1'=>$datos['apellido1'], 'apellido2'=>$datos['apellido2'], 'correo'=>$datos['correo'], 'tipo'=>$datos['id_tipo'],   'id_instancia'=>$datos['id_instancia'] )
					);
				}				
               else {
